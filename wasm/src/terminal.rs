...
impl Terminal {
    pub fn new() -> Self {
        let size = unsafe { (ffi::get_width() * ffi::get_height()) as usize };
        ...
        unsafe {
            ffi::set_bufs(
                chars.as_mut_ptr(),
                style.as_mut_ptr(),
                fg_colour.as_mut_ptr(),
                bg_colour.as_mut_ptr(),
            );
        }
    ...
    pub fn draw_grid(&mut self, grid: &Grid<Cell>) {
        for (cell, chars_entry, style_entry, fg_colour_entry, bg_colour_entry) in izip! {
            ...
            *style_entry = if cell.bold { BOLD_BIT } else { 0 } | if cell.underline {
                UNDERLINE_BIT
            } else {
                0
            };
